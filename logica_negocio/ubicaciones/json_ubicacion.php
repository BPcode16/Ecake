...
if (isset($_POST['ingreso_datos']) && $_POST['ingreso_datos'] == "si_actualizalo") {
    ...
    if ($iddepartamento > 0) {
        $where = "and m.iddepartamento = $iddepartamento";
    } else {
        $where = "";
    }
    $sql = "SELECT m.iddepartamento, m.idmunicipio, m.nombre as nombre1, d.nombre as nombre, m.costoenvio
        ...
        ON m.iddepartamento = d.iddepartamento WHERE costoenvio is not NULL $where";
    ...
    if ($_POST['departamento'] == 0) {
        $resultado = $modelo->get_todos("tbl_municipio", "WHERE costoenvio is not NULL");

        if ($resultado[4] == 0) {
            $cadena = "
        <button id='btn_eliminar' class='btn btn-secondary ml-auto' disabled>Remover envíos al país completo</button>";

            echo  $cadena . "";
        } else {
            $cadena = "
            <button id='btn_eliminar' class='btn btn-secondary ml-auto'>Remover envíos al país completo</button>";
